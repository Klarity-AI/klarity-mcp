---
title: "Drive AI transformation"
description: "The motion that turns how your organization runs into skills, agents, and automations."
icon: "compass"
---

The motion has three phases. [Discover](/transformation/company-brain) captures how work happens, [Structure](/transformation/company-brain#structure) makes sense of it, and Improve builds skills and agents on top. Klarity runs this as a continuous loop rather than a one-time project. See [Discover → Structure → Improve](/concepts/discover-structure-improve) for the model itself; these pages are how you run it.

Start by naming a high-level business objective and the technology stack you want to build within. Klarity respects the enterprise stack and won't recommend a competitor's tools.

## The Company Brain

These pages use **Company Brain** for the living map of how your organization actually runs. It is what the portal's two intelligence layers add up to: the [Process Index](/concepts/process-index), which holds every process with its steps, dependencies, and versions, and [Artifacts](/concepts/artifacts), the source documents and recordings behind them.

Everything you build draws on it. That is what separates a build grounded in how your business actually works from one built against a generic template.

## Where to start

<Columns cols={2}>
  <Card title="Set the objective" icon="bullseye" href="/transformation/objective">
    Name the outcome in business terms before choosing any technology.
  </Card>
  <Card title="Build the Company Brain" icon="brain" href="/transformation/company-brain">
    Turn on Companion, then spend time in what it captures.
  </Card>
  <Card title="Pick the right use case" icon="list-check" href="/transformation/use-case-selection">
    Choosing the right problem beats choosing how to build it.
  </Card>
  <Card title="What you can build" icon="cubes" href="/transformation/what-you-can-build">
    Skills, Integration Flows, Managed Agents, and context bundles.
  </Card>
  <Card title="Where you build" icon="table-columns" href="/transformation/where-you-build">
    Four surfaces, split across a non-technical and a technical track.
  </Card>
  <Card title="Keep agents current" icon="arrows-rotate" href="/transformation/maintain-agents">
    A build is a product. Refresh it as the business moves.
  </Card>
</Columns>
