...
<Steps>
  ...
  <Step title="Drill if needed">
    If `fetch`'s summary isn't enough, drop into [Process Index tools](/tools/process-index) for the richer payload, or chain into [observations](/tools/observations-and-changes) and the [Context Graph](/tools/context-graph) for evidence and relationships.
  </Step>
</Steps>

## When to skip `search`

If the user has already named a specific process or you have the ID from a prior call, go straight to `fetch`. If the user wants to *browse* rather than search, start with [`get_process_hierarchy_tree`](/tools/process-index) instead.
