...
The first command points Claude Code at this repo's `.claude-plugin/marketplace.json` catalog. The second installs plugin `klarity` from marketplace `klarity` (same name — the marketplace exposes a single plugin).
