...
<Steps>
  ...
  <Step title="Build the work queue">
    Call [`get_process_hierarchy_tree`](/tools/process-index) — the whole tree, or filtered to the value stream of interest. The leaves become your work queue.
  </Step>
  <Step title="Fan out in parallel across leaves">
    ...
    - [`list_process_observations`](/tools/observations-and-changes) — exception volume per process.

    <Tip>
      The MCP is stateless per call. There's no penalty for parallel reads — fan out aggressively.
    </Tip>
  </Step>
  <Step title="Hunt high-leverage patterns in the graph">
    Call [`search_knowledge_graph`](/tools/context-graph) for patterns: "manual approval", "duplicate entry", "data re-keying", "exception handling", "swivel-chair workflow". Then [`explore_graph_neighbors`](/tools/context-graph) on each pattern entity to see which processes and systems touch it.
  </Step>
  <Step title="Check workspace metadata">
    Call [`get_attribute_configurations`](/tools/workspace) to look for "automation status", "system of record", "control level", or similar attributes that help rank candidates.
  </Step>
  <Step title="Avoid duplicating prior Advisor work">
    Call [`list_objective_findings`](/tools/objectives). If Advisor has already analyzed this area, call [`get_objective_context_bundle`](/tools/objectives) to load prior thinking and build on it. If not, surface a fresh candidate set.
  </Step>
  <Step title="Confirm each top candidate">
    For each top candidate:
    - [`get_process_observations`](/tools/observations-and-changes) — confirm the deviation or manual pattern with concrete evidence.
    - [`get_downstream_dependencies`](/tools/context-graph) — first cut at blast radius.
  </Step>
